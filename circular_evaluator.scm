...
(define (let->combination exp env)
  ;;extract the parameters out.
  (let ((var-exps (segreg (params-let exp))) (body (cddr exp)))
  (let  ((lamb (make-lambda (car (var-exps)) body)) (exp-list (cdr var-exps)))
   (eval (cons lamb exp-list) env)
    )
  )
  )
...
;;conversion is as follows: (define (for-iter i clause inc) begin (body) for-iter (+ i inc)) (for-iter 0 clause 1)
...
(define (eval-and seq env)
  ;;get the exps and get them through the evaluator nand record their values and if all of them are true then get the true result or else false.
  (let ((first (and-first seq)))
    (if (eval first env)
	(if (last-seq-and? seq)
	    ('true)
	    (eval-and (and-rest-seq seq) env))
	('false)
	)
    )
  )
...
(define (eval exp env)
  (cond ((self-evaluating? exp) exp)
        ((variable? exp)(display "variable") (lookup-variable-value exp env))
        ((quoted? exp)(display "quoted") (text-of-quotation exp))
        ((assignment? exp)(display "ass") (eval-assignment exp env))
        ((definition? exp)(display "def") (eval-definition exp env))
        ((if? exp)(display "if") (eval-if exp env))
        ((lambda? exp)
	 (display "lambda")
